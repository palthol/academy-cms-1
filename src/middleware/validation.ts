...
// Common validation error handler that matches Express's middleware signature
const handleValidationErrors = (req: Request, res: Response, next: NextFunction): void => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    res.status(400).json({ errors: errors.array() });
    return; // Return without value instead of returning the response
  }
  next();
};

// Type-safe validator array creator
const createValidator = (validations: ValidationChain[]): (RequestHandler)[] => {
  return [...validations, handleValidationErrors as any];
};

// Middleware for validating client registration
export const validateClientRegistration = createValidator([
...
export const validateEmployeeCreation = createValidator([
  body('name').isString().notEmpty().withMessage('Name is required'),
  body('email').isEmail().withMessage('Valid email is required'),
...
export const validateSubscriptionCreation = createValidator([
  body('name').isString().notEmpty().withMessage('Name is required'),
...
export const validateTrainingSessionCreation = createValidator([
...
export const validatePaymentRecordCreation = createValidator([
  body('clientId').isInt().withMessage('Client ID must be an integer'),
